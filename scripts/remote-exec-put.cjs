...
if (!password || !remoteRoot || !files.length) {
  console.error("Usage: SSH_PASS=... REMOTE_ROOT=/remote/root node scripts/remote-exec-put.cjs file...");
  process.exit(1);
}
...
function putFile(conn, localFile, remoteFile) {
  return new Promise((resolve, reject) => {
    ...
    const command = `mkdir -p ${shellQuote(remoteDir)} && base64 -d > ${shellQuote(remoteFile)}`;
    conn.exec(command, (err, stream) => {
      ...
      stream.on("close", (code) => {
        if (code) return reject(new Error(stderr || `upload failed: ${remoteFile}`));
        resolve();
      });
      stream.end(fs.readFileSync(localFile).toString("base64"));
    });
  });
}
