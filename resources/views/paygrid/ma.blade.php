...
@if($active === 'overview')
    @php
        $overviewCards = [
            ...
            'pending_settlement' => ['label' => 'Pending Settlement', 'value' => $money($summary['pending_settlement']), 'class' => 'pending', 'note' => 'Klik untuk list pending'],
            'total_settlement' => ['label' => 'Total Settlement', 'value' => $money($summary['total_settlement']), 'class' => '', 'note' => 'Net dari trx sukses'],
            'trx_total' => ['label' => 'Total Transaksi', 'value' => number_format($summary['trx_total'], 0, ',', '.'), 'class' => '', 'note' => 'Semua status'],
    ...
    @endphp
    <section class="grid qris-metrics history-metrics section">
    ...
    <section class="card qris-panel section"><div class="qris-toolbar"><h2>Analytics Top 10 Terendah</h2><span class="muted">Periode {{ $periodLabel }}</span><div class="ma-tabs"><button class="btn compact-btn active" type="button" data-ma-tab="toko">Toko</button><button class="btn compact-btn" type="button" data-ma-tab="agen">Agen</button></div></div><div data-ma-panel="toko"><table class="table qris-table"><thead><tr><th>Toko</th><th>Total TRX</th><th>Volume Sukses</th></tr></thead><tbody>@forelse($storeRanking as $row)<tr><td>{{ $row->merchant?->name ?: '-' }}</td><td>{{ $row->trx }}</td><td>{{ $money($row->volume) }}</td></tr>@empty<tr><td colspan="3" class="empty">Belum ada data toko.</td></tr>@endforelse</tbody></table></div><div data-ma-panel="agen" hidden><table class="table qris-table"><thead><tr><th>Agen</th><th>Total TRX</th><th>Volume Sukses</th></tr></thead><tbody>@forelse($agentRanking as $row)<tr><td>{{ $row->agent?->name ?: '-' }}</td><td>{{ $row->trx }}</td><td>{{ $money($row->volume) }}</td></tr>@empty<tr><td colspan="3" class="empty">Belum ada data agen.</td></tr>@endforelse</tbody></table></div></section>
    <section class="card qris-panel ma-store-panel section"><div class="qris-toolbar"><h2>List Toko</h2><span class="muted">Top 10 trx terendah | {{ $periodLabel }}</span></div><div class="table-wrap"><table class="table qris-table ma-store-summary-table"><thead><tr><th>Nama Toko</th><th>Agen</th><th>Total Transaksi</th><th>Volume Sukses</th><th>Pending Balance</th><th>Settlement</th></tr></thead><tbody>@forelse($storeSummaries as $store)<tr><td><strong>{{ $store['name'] }}</strong></td><td>{{ $store['agent'] }}</td><td>{{ number_format($store['trx_total'], 0, ',', '.') }}</td><td>{{ $money($store['volume_success']) }}</td><td>{{ $money($store['pending_balance']) }}</td><td>{{ $money($store['settlement']) }}</td></tr>@empty<tr><td colspan="6" class="empty">Belum ada toko.</td></tr>@endforelse</tbody></table></div></section>
...
@if($active === 'report')
    ...
    @if($selectedStore)
        @php
            $trxCollection = collect($transactions->items());
        @endphp
        <section class="grid qris-metrics section"><div class="card pad qris-metric"><span>Total TRX Halaman</span><strong>{{ number_format($trxCollection->count(), 0, ',', '.') }}</strong><small>{{ $selectedStore->name }}</small></div><div class="card pad qris-metric success"><span>Sukses Halaman</span><strong>{{ number_format($trxCollection->where('status', 'success')->count(), 0, ',', '.') }}</strong><small>{{ $periodLabel }}</small></div><div class="card pad qris-metric pending"><span>Pending Halaman</span><strong>{{ number_format($trxCollection->where('status', 'pending')->count(), 0, ',', '.') }}</strong><small>Per page 25</small></div><div class="card pad qris-metric primary"><span>Volume Halaman</span><strong>{{ $money($trxCollection->sum('amount')) }}</strong><small>Transaksi toko</small></div></section>
        <section class="card qris-panel section"><div class="qris-toolbar"><h2>Transaksi {{ $selectedStore->name }}</h2><span class="muted">Per page 25 | {{ $periodLabel }}</span></div><div class="table-wrap"><table class="table qris-table ma-report-table"><thead><tr><th>Tanggal</th><th>Toko</th><th>Agen</th><th>Status</th><th>Amount</th><th>Reference</th><th>RRN</th><th>Payment ID</th><th>Net</th><th>Settlement</th><th>Sumber TRX</th></tr></thead><tbody>@forelse($transactions as $trx)<tr data-agent-id="{{ $trx->merchant?->agent?->id }}" data-store-id="{{ $trx->merchant?->id }}"><td class="time-cell">{{ $trx->submitted_at?->timezone('Asia/Jakarta')->format('d/m/y') }}<span>{{ $trx->submitted_at?->timezone('Asia/Jakarta')->format('H.i') }}</span></td><td>{{ $trx->merchant?->name ?: '-' }}</td><td>{{ $trx->merchant?->agent?->name ?: '-' }}</td><td><span class="badge {{ $badge($trx->status) }}">{{ $trx->status }}</span></td><td>{{ $money($trx->amount) }}</td><td>{{ $trx->customer_reference ?: $trx->gateway_ref_id ?: '-' }}</td><td>{{ $trx->rrn ?: '-' }}</td><td>{{ $trx->payment_id ?: '-' }}</td><td>{{ $money($trx->net_amount) }}</td><td>{{ $trx->status === 'success' ? 'Settleable' : 'Pending' }}</td><td>{{ $trx->data_source }}</td></tr>@empty<tr><td colspan="11" class="empty">Belum ada transaksi.</td></tr>@endforelse</tbody></table></div><div class="qris-pagination pad"><div class="pager-summary">Showing {{ $transactions->firstItem() ?? 0 }} to {{ $transactions->lastItem() ?? 0 }}</div><div class="pager-links">@if($transactions->onFirstPage())<span class="pager disabled">Prev</span>@else<a class="pager" href="{{ $transactions->previousPageUrl() }}">Prev</a>@endif @if($transactions->hasMorePages())<a class="pager" href="{{ $transactions->nextPageUrl() }}">Next</a>@else<span class="pager disabled">Next</span>@endif</div></div></section>
    @else
        <section class="card pad section"><h2>Pilih Toko</h2><p class="muted">Transaksi tidak dimuat dulu supaya report tetap ringan. Pilih agen, lalu pilih toko untuk melihat transaksi paginated.</p></section>
    @endif
@endif
...
@if($active === 'fee')
    ...
    <section class="card qris-panel section"><div class="qris-toolbar"><h2>Pembagian Fee Per Toko</h2></div><table class="table qris-table"><thead><tr><th>Toko</th><th>Merchant Fee</th><th>Base HG</th><th>Engine</th><th>Settlement</th><th>Agent</th><th>MA Fee</th><th>Formula</th></tr></thead><tbody>@foreach($merchants as $m)<tr><td><strong>{{ $m->name }}</strong></td><td>{{ $pct($m->merchant_mdr_percent) }}</td><td>{{ $pct($m->base_mdr_percent) }}</td><td>{{ $pct($m->connection_fee_percent) }}</td><td>{{ $pct($m->settlement_fee_percent) }}</td><td>{{ $pct($m->agent_fee_percent) }}</td><td><strong>{{ $pct($m->ma_fee_percent) }}</strong></td><td class="muted">Merchant - Base - Engine - Settlement - Agent</td></tr>@endforeach</tbody></table></section>
@endif
    ...
    @foreach($registrations as $r)
        @php
            ...
            $detailRows = [
                ...
                'Pay In Fee' => $payinFee.'%',
                'Second TRX Fee' => ($payload['second_transaction_fee_percentage'] ?? 0).'%',
                'Third TRX Fee' => ($payload['third_transaction_fee_percentage'] ?? 0).'%',
                'Disbursement Fee' => $payload['withdrawal_fee'] ?? '-',
                'Disbursement Fee %' => ($payload['withdrawal_fee_percentage'] ?? 0).'%',
            ];
        @endphp
...
@if($active === 'stores')
    <section class="card qris-panel section ma-store-list-panel">
        ...
        @forelse($merchants as $m)
            ...
            <div class="approval-modal" id="store-fee-{{ $m->id }}" hidden><div class="approval-modal-card"><div class="qris-toolbar"><div><h2>Edit Fee</h2><p class="muted" style="margin:4px 0 0">{{ $m->name }}</p></div><button class="btn compact-btn approval-detail-close" type="button">Tutup</button></div><form method="post" action="{{ route('ma.stores.fee.update', $m) }}" class="form-grid pad">@csrf<label>Merchant MDR %<input name="merchant_mdr_percent" value="{{ $pctInput($m->merchant_mdr_percent) }}" required></label><label>Base HG %<input name="base_mdr_percent" value="{{ $pctInput($m->base_mdr_percent) }}" required></label><label>Pay In Fee %<input name="payin_fee_percent" value="{{ $pctInput($m->payin_fee_percent) }}" required></label><label>Settlement/Disbursement %<input name="settlement_fee_percent" value="{{ $pctInput($m->settlement_fee_percent) }}" required></label><label>MA Fee %<input name="ma_fee_percent" value="{{ $pctInput($m->ma_fee_percent) }}" required></label><label>Agent Fee %<input name="agent_fee_percent" value="{{ $pctInput($m->agent_fee_percent) }}" required></label><div><button class="btn primary">Simpan Fee</button></div></form></div></div></td></tr>
        @empty
