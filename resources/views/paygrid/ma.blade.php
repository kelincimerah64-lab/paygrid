...
@if($active === 'fee')
    ...
    <section class="card qris-panel section"><div class="qris-toolbar"><h2>Pembagian Fee Per Toko</h2></div><table class="table qris-table"><thead><tr><th>Toko</th><th>Menu Fee</th><th>Merchant MDR</th><th>Estimasi MDR Toko</th><th>Agent</th><th>Estimasi Fee Agen</th><th>MA Fee</th><th>Estimasi Fee MA</th><th>Detail</th></tr></thead><tbody>@foreach($merchants as $m)<tr><td><strong>{{ $m->name }}</strong></td><td>{{ $m->fee_menu ? ($feeMenus->optionsFor('merchant')[$m->fee_menu]['label'] ?? $m->fee_menu) : '-' }}</td><td>{{ $pct($m->merchant_mdr_percent) }}</td><td>{{ $money($m->merchant_fee_amount ?? 0) }}</td><td>{{ $pct($m->agent_fee_percent) }}</td><td>{{ $money($m->agent_fee_amount ?? 0) }}</td><td><strong>{{ $pct($m->ma_fee_percent) }}</strong></td><td><strong>{{ $money($m->ma_fee_amount ?? 0) }}</strong></td><td><button class="btn compact-btn approval-detail-open" type="button" data-approval-detail="fee-tab-store-{{ $m->id }}">Detail</button>
