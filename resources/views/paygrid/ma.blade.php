...
@if($active === 'fee')
    ...
    <section class="card qris-panel section"><div class="qris-toolbar"><h2>Pembagian Fee Per Toko</h2></div><table class="table qris-table"><thead><tr><th>Toko</th><th>Menu Fee</th><th>Merchant MDR</th><th>Agent</th><th>MA Fee</th><th>Detail</th></tr></thead><tbody>@foreach($merchants as $m)<tr><td><strong>{{ $m->name }}</strong></td><td>{{ $m->fee_menu ? ($feeMenus->optionsFor('merchant')[$m->fee_menu]['label'] ?? $m->fee_menu) : '-' }}</td><td>{{ $pct($m->merchant_mdr_percent) }}</td><td>{{ $pct($m->agent_fee_percent) }}</td><td><strong>{{ $pct($m->ma_fee_percent) }}</strong></td><td><button class="btn compact-btn approval-detail-open" type="button" data-approval-detail="fee-tab-store-{{ $m->id }}">Detail</button>
