...
@if($active === 'fee')
    ...
    <section class="card qris-panel section"><div class="qris-toolbar"><h2>Pembagian Fee Per Toko</h2></div><table class="table qris-table ma-fee-split-table"><thead><tr><th>Toko</th><th>Menu Fee</th><th>Merchant MDR</th><th>Estimasi MDR Toko</th><th>Agent</th><th>Estimasi Fee Agen</th><th>MA Fee</th><th>Estimasi Fee MA</th><th>Detail</th></tr></thead><tbody>@foreach($merchants as $m)<tr><td><strong>{{ $m->name }}</strong></td><td>{{ $m->fee_menu ? ($feeMenus->optionsFor('merchant')[$m->fee_menu]['label'] ?? $m->fee_menu) : '-' }}</td><td>{{ $pct($m->merchant_mdr_percent) }}</td><td>{{ $money($m->merchant_fee_amount ?? 0) }}</td><td>{{ $pct($m->merchant_mdr_percent - $m->agent_fee_percent) }}</td><td>{{ $money($m->agent_fee_amount ?? 0) }}</td><td><strong>{{ $pct($m->merchant_mdr_percent - $m->ma_fee_percent) }}</strong></td><td><strong>{{ $money($m->ma_fee_amount ?? 0) }}</strong></td><td><button class="btn compact-btn approval-detail-open" type="button" data-approval-detail="fee-tab-store-{{ $m->id }}">Detail</button>
        <div class="approval-modal" id="fee-tab-store-{{ $m->id }}" hidden><div class="approval-modal-card"><div class="qris-toolbar"><div><h2>Detail Fee Menu</h2><p class="muted" style="margin:4px 0 0">{{ $m->name }}</p></div><button class="btn compact-btn approval-detail-close" type="button">Tutup</button></div>@include('paygrid.partials.fee-menu-rates-readonly', ['role' => 'merchant', 'feeMenus' => $feeMenus, 'rates' => $m->fee_menu_rates ?? []])</div></div></td></tr>@endforeach</tbody></table></section>
@endif
...
@if($active === 'approval')
    ...
    @foreach($registrations as $r)
        @php
            ...
            $detailRows = [
                'Engine Name' => $r->engine_name ?: ($payload['engine_name'] ?? '-'),
                'Tipe Merchant' => strtoupper($r->merchant_type ?: ($payload['merchant_type'] ?? '-')),
                'PIC Email' => $payload['pic_email'] ?? '-',
                'PIC Telegram' => $payload['pic_telegram'] ?? '-',
                'Finance Email' => $payload['finance_email'] ?? '-',
                'Finance Telegram' => $payload['finance_telegram'] ?? '-',
                'CS Email' => $payload['cs_email'] ?? '-',
                'CS Telegram' => $payload['cs_telegram'] ?? '-',
                'IP Dashboard' => $payload['ip_dashboard'] ?? '-',
                'IP Finance' => $payload['ip_finance'] ?? '-',
                ...
                'Transaction Gateway ID' => $payload['transaction_gateway_ids'] ?? '-',
                'Withdrawal Gateway ID' => $payload['withdrawal_gateway_ids'] ?? '-',
