    ...
    <tr><td><strong>{{ $m->name }}</strong></td><td>{{ $menuLabel }}</td><td>{{ $pct($mdrToko) }}</td><td>{{ $pct($basedFeeMa) }}</td><td>{{ $pct($marginAgent) }}</td><td>{{ $money($m->agent_fee_amount ?? 0) }}</td><td>{{ $pct($marginMa) }}</td><td>{{ $money($m->ma_fee_amount ?? 0) }}</td><td><span class="badge {{ $isValidFloor ? 'ok' : 'warn' }}">{{ $isValidFloor ? 'Valid' : 'Gak Sinkron' }}</span></td><td><button class="btn compact-btn approval-detail-open" type="button" data-approval-detail="fee-tab-store-{{ $m->id }}">Detail</button>
        ...
        <div class="table-wrap"><table class="table qris-table">
            ...
            <tbody><tr>
                <td>{{ $menuLabel }}</td>
                <td>{{ $pct($mdrToko) }}</td>
                <td>{{ $pct($basedFeeMa) }}</td>
                <td>{{ $pct($basedFeeAgent) }}</td>
                <td>{{ $pct($mdrToko - $basedFeeMa) }}</td>
                <td>{{ $pct($marginAgent) }}</td>
                <td>{{ $pct($marginMa) }}</td>
                <td>{{ number_format($m->volume_trx ?? 0, 0, ',', '.') }}</td>
                <td>{{ $money($m->ma_fee_amount ?? 0) }}</td>
